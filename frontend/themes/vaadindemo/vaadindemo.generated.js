...
window.Vaadin.Flow['_vaadintheme_vaadindemo_globalCss'] = window.Vaadin.Flow['_vaadintheme_vaadindemo_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_vaadindemo_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_vaadindemo_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_vaadindemo_globalCss'].push(target);
  }
  if (!document['_vaadintheme_vaadindemo_componentCss']) {
    registerStyles(
      'button',
      css`
        ${unsafeCSS(buttonCss.toString())}
      `
    );
    
    document['_vaadintheme_vaadindemo_componentCss'] = true;
  }
